...
Item {
    ...
    property int vachchialon: 20
    property int vachchianho: 5
    ...
    //Vach chia
    Repeater {
        model: (root.maxSpeed / root.vachchianho) +1
        ...
        Rectangle {
            ...
            transform: Rotation {
                origin.x: width / 2
                origin.y: m_radius
                angle: root.startAngle + (value / root.maxSpeed) * root.sweepAngle
            }
        }
    }
    //So chia
    Repeater
    {
        model: (root.maxSpeed / root.vachchialon) +1
        Text {
